...
inCSV='/mnt/ramses25/mar/gaiaSkinnyishDR3PMERR.csv'
#inCSV='/mnt/ramses25/mar/gaiaSkinnyishDR3PMERR_1000000.csv'
outGood='/unas/mar/gaiaDR3GoodPMERR.csv'
outBad='/unas/mar/gaiaDR3BadPMERR.csv'
...
for csv_chunk in pd.read_csv(inCSV, header=None,chunksize=chunkSize,names=colNames,dtype=colTypes,skiprows=skipRows):
...
#    for index, row in enumerate(csv_chunk.itertuples(),1):
    ...
    phot_bp_mean_flux_over_error=csv_chunk.loc[:,('phot_bp_mean_flux_over_error')].values
    phot_g_mean_flux_over_error=csv_chunk.loc[:,('phot_g_mean_flux_over_error')].values
    phot_rp_mean_flux_over_error=csv_chunk.loc[:,('phot_rp_mean_flux_over_error')].values



    bp_mean_mag_err=np.where(np.isnan(phot_bp_mean_flux_over_error), phot_bp_mean_flux_over_error, np.sqrt( np.square(2.5/np.log(10.0)*1/phot_bp_mean_flux_over_error)+np.square(0.0027901700) )   )
    g_mean_mag_err=np.where(np.isnan(phot_g_mean_flux_over_error), phot_g_mean_flux_over_error, np.sqrt( np.square(2.5/np.log(10.0)*1/phot_g_mean_flux_over_error)+np.square(0.0027553202) )   )
    rp_mean_mag_err=np.where(np.isnan(phot_rp_mean_flux_over_error), phot_rp_mean_flux_over_error, np.sqrt( np.square(2.5/np.log(10.0)*1/phot_rp_mean_flux_over_error)+np.square(0.0037793818) )   )
...
#    for pp in range(0,6):
#        print("### g_mean_mag ",phot_g_mean_mag[pp]," phot_bp_rp_excess_factor ",phot_bp_rp_excess_factor[pp], "corr_phot_bp_rp_excess_factor ",corr_phot_bp_rp_excess_factor[pp], "  bp_rp ",bp_rp[pp])
#        print("### ")
...
#        print row,row.source_id,"\n"
        ...
        flag=[2,2,2]
        conds=[0,0,0]
        mags=[row.phot_bp_mean_mag,row.phot_g_mean_mag,row.phot_rp_mean_mag]
...
#        print "PMFOES",pmfoes
        ...
        try:
            ...
            sig_avg = math.sqrt(sig_maj * sig_min)

            


            if (not math.isnan(row.pmra_error) and not math.isnan(row.pmdec_error) and not math.isnan(row.pmra_pmdec_corr)  ) :
                sig_maj_pm=math.sqrt(0.5*(row.pmra_error**2 + row.pmdec_error**2 + math.sqrt((row.pmra_error**2-row.pmdec_error**2)**2 \
                                                                                             + 4*(row.pmra_pmdec_corr * row.pmra_error * row.pmdec_error)**2)))
                sig_min_pm=math.sqrt(0.5*(row.pmra_error**2 + row.pmdec_error**2 - math.sqrt((row.pmra_error**2-row.pmdec_error**2)**2 \
                                                                                             + 4*(row.pmra_pmdec_corr * row.pmra_error * row.pmdec_error)**2)))
                sig_avg_pm=math.sqrt(sig_maj_pm * sig_min_pm)

            ra=row.ra
            dec=row.dec

            if (not math.isnan(row.pmra) and row.ra < 360.0 and row.ra >= 0 and \
                row.dec <= 90.0 and row.dec >= -90.0 and not math.isnan(row.pmdec)):
...
#                flag=-999
        ...
        for mm in range(len(mags)):
            if np.isnan(mags[mm]):
                pmfoes[mm] = np.nan
            
        if (include):
#            goodRows.append([row.source_id,row.designation,tra,tdec,sig_avg/1000.0,mags[0],mags[1],mags[2],bestIndex,bp_mean_mag_err[rn],g_mean_mag_err[rn],rp_mean_mag_err[rn],hpi[rn],bestFlag,flag[0],flag[1],flag[2],row.ra,row.dec,row.phot_bp_mean_mag,row.phot_g_mean_mag,row.phot_rp_mean_mag])
            goodRows.append([row.source_id,row.designation,row.ra,row.dec,sig_avg/1000.0,mags[0],mags[1],mags[2],bestIndex,pmfoes[0],pmfoes[1],pmfoes[2],row.pmra/1000.0,row.pmdec/1000.0,sig_avg_pm/1000.0,hpi[rn],bestFlag,flag[0],flag[1],flag[2],row.ra,row.dec,row.phot_bp_mean_mag,row.phot_g_mean_mag,row.phot_rp_mean_mag])
            rowsWritten += 1
            goodCounts[hpi[rn]] += 1
        else :
            badRows.append([row.source_id,row.designation,row.ra,row.dec,sig_avg/1000.0,mags[0],mags[1],mags[2],bestIndex,pmfoes[0],pmfoes[1],pmfoes[2],row.pmra/1000.0,row.pmdec/1000.0,sig_avg_pm/1000.0,hpi[rn],bestFlag,flag[0],flag[1],flag[2],row.ra,row.dec,row.phot_bp_mean_mag,row.phot_g_mean_mag,row.phot_rp_mean_mag])
#            badRows.append([row.source_id,row.designation,tra,tdec,sig_avg/1000.0,mags[0],mags[1],mags[2],bestIndex,bp_mean_mag_err[rn],g_mean_mag_err[rn],rp_mean_mag_err[rn],hpi[rn],bestFlag,flag[0],flag[1],flag[2],row.ra,row.dec,row.phot_bp_mean_mag,row.phot_g_mean_mag,row.phot_rp_mean_mag])
            badCounts[hpi[rn]] += 1
    #goodDF=pd.DataFrame(goodRows

    logger.info("ENDINGFLAGGING=")
    outCols=['source_id','designation','tra','tdec','sig_avg','mag0','mag1','mag2','bestIndex','bp_mean_flux_over_error','g_mean_flux_over_error','rp_mean_flux_over_error','pmra','pmdec','sig_avg_pm','hpi','bestFlag','flag0','flag1','flag2','ra','dec','omag0','omag1','omag2']
    goodDF=pd.DataFrame(goodRows,columns=outCols)
    goodDF=goodDF.round({'tra': 9, 'tdec': 8, 'sig_avg': 8, 'mag0': 4, 'mag1': 4, 'mag2': 4, 'bp_mean_flux_over_error': 6 , 'g_mean_flux_over_error': 6, 'rp_mean_flux_over_error': 6,'pmra':9, 'pmdec':9, 'sig_avg_pm': 10, 'ra': 9, 'dec': 8, 'omag0': 4, 'omag1': 4, 'omag2': 4})
    print (goodDF.dtypes)

    badDF=pd.DataFrame(badRows,columns=outCols)
    badDF=badDF.round({'tra': 9, 'tdec': 8, 'sig_avg': 8, 'mag0': 4, 'mag1': 4, 'mag2': 4, 'bp_mean_flux_over_error': 6 , 'g_mean_flux_over_error': 6, 'rp_mean_flux_over_error': 6, 'pmra':9, 'pmdec':9, 'sig_avg_pm': 9, 'ra': 9, 'dec': 8, 'omag0': 4, 'omag1': 4, 'omag2': 4})
#    badDF=badDF.round({'tra': 9, 'tdec': 8, 'sig_avg': 8, 'mag0': 4, 'mag1': 4, 'mag2': 4, 'bp_mean_mag_err': 6 , 'g_mean_mag_err': 6, 'rp_mean_mag_err': 6, 'ra': 9, 'dec': 8, 'omag0': 4, 'omag1': 4, 'omag2': 4})
...
np.savetxt("/unas/mar/gaiaDR3TotCountswithMagErrors.csv", totCounts, delimiter=",")
np.savetxt("/unas/mar/gaiaDR3GoodCountswitMaghErrors.csv", goodCounts, delimiter=",")
np.savetxt("/unas/mar/gaiaDR3BadCountswithMagErrors.csv", badCounts, delimiter=",")
